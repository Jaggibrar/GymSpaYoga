...
CREATE POLICY IF NOT EXISTS "Owners can view business chat rooms" ON chat_rooms
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Owners can view trainer chat rooms" ON chat_rooms
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Owners can update business chat rooms" ON chat_rooms
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM business_profiles bp
    WHERE bp.user_id = auth.uid() AND bp.id = chat_rooms.business_id
...
CREATE POLICY IF NOT EXISTS "Owners can update trainer chat rooms" ON chat_rooms
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM trainer_profiles tp
    WHERE tp.user_id = auth.uid() AND tp.id = chat_rooms.trainer_id
...
CREATE POLICY IF NOT EXISTS "Owners can view messages in business rooms" ON chat_messages
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Owners can view messages in trainer rooms" ON chat_messages
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM chat_rooms cr
...
CREATE POLICY IF NOT EXISTS "Owners can send messages in business rooms" ON chat_messages
...
CREATE POLICY IF NOT EXISTS "Owners can send messages in trainer rooms" ON chat_messages
FOR INSERT WITH CHECK (
  (auth.uid() = sender_id) AND EXISTS (
    SELECT 1 FROM chat_rooms cr
    JOIN trainer_profiles tp ON tp.id = cr.trainer_id
...
CREATE POLICY IF NOT EXISTS "Participants can mark messages read" ON chat_messages
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM chat_rooms cr
