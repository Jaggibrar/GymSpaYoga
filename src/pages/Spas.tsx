...
import { Search, MapPin, Star, Clock, Waves, IndianRupee, Crown, Diamond } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50">
      ...
      <PageHero
        ...
        backgroundImage="https://images.unsplash.com/photo-1544161515-4ab6ce6db874?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80"
      />
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 md:p-6 shadow-xl mb-6 md:mb-8">
          <div className="flex flex-col gap-3 md:flex-row md:gap-4">
            ...
            <Select value={tierFilter} onValueChange={setTierFilter}>
              <SelectTrigger className="h-12 md:h-14 mobile-text md:text-lg border-0 focus:ring-2 focus:ring-blue-500">
            ...
            {spas.map((spa) => (
              <Card key={spa.id} className="group hover:shadow-2xl transition-all duration-300 hover:-translate-y-2">
                <div className="relative overflow-hidden h-40 md:h-48">
                  ...
                  />
                  <Badge className={`absolute top-3 md:top-4 right-3 md:right-4 bg-gradient-to-r ${getTierColor(spa.tier!)} text-white`}>
                    {getTierIcon(spa.tier!)}
                    <span className="ml-1 capitalize">{spa.tier}</span>
                  </Badge>
                </div>
                ...
                <CardContent className="pt-0 p-4 md:p-6">
                  ...
                  <div className="flex items-center justify-between mb-3 md:mb-4">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs md:text-sm text-gray-500">(89)</span>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-right">
                      <p className="text-lg md:text-xl font-bold text-blue-600">
                        {spa.monthly_price ? `₹${spa.monthly_price}/month` : spa.session_price ? `₹${spa.session_price}/session` : "Contact for pricing"}
                      </p>
                    </div>
                    <Link to={`/spa/${spa.id}`}>
                      <Button 
